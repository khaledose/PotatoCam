...
public class LensBlur{
    ...
    private float bloom = 8;
    private float bloomThreshold = 220;
    private float angle = 60;
    private int sides = 12;
    public LensBlur(float radius,float bloomThreshold,float bloom){
        this.radius = radius;
    ...
    public Bitmap filter(Bitmap src,Bitmap deeplabSeg) {
...
//                System.out.println("Tile: "+tileX+" "+tileY+" "+tileWidth+" "+tileHeight);
                ...
                src.getPixels(rgb,fy*w+fx,w,tx,ty,tw,th);
                deeplabSeg.getPixels(seg,fy*w+fx,w,tx,ty,tw,th);
                ...
                for ( int y = 0; y < h; y++ ) {
                    ...
                    for ( int x = 0; x < w; x++ ) {
                        ...
                        if ( imageX < 0 )
                            ...
                            k = x;
                        k += j;
                        ar[0][i] = Color.alpha(rgb[k]);
                        int r = Color.red(rgb[k]);
                        int g = Color.green(rgb[k]);
                        int b = Color.blue(rgb[k]);
...
//                        float b = (rgb[k] & 0xff);
                        ...
                        // Bloom...
                        if ( r > bloomThreshold && Color.red(seg[k])<20)
                            r *= bloom;
//							r = bloomThreshold + (r-bloomThreshold) * bloom;
                        if ( g > bloomThreshold && Color.red(seg[k])<20)
                            g *= bloom;
//							g = bloomThreshold + (g-bloomThreshold) * bloom;
                        if ( b > bloomThreshold && Color.red(seg[k])<20)
                            b *= bloom;
//							b = bloomThreshold + (b-bloomThreshold) * bloom;
                ...
                //Log.e("test",tw+" "+th);
                if(th>=0&&tw>=0)
                dst.setPixels(rgb,iradius*w+iradius,w,tx,ty,tw,th);
            }
        }
        return dst;
    }
